...
class SimpleQuestGenerator:
    ...
    def generate_quest(self):
        print(f"Quest Name: {self.quest_name}")
        print(f"Quest Description: {self.quest_description}")
        print(f"Quest Reward: {self.quest_reward}")
    ...
    async def invoke_kernel(self, kernel, plugin_function, genre, difficulty):
        plugin = await kernel.invoke(plugin_function,
                                     sk.KernelArguments(genre=genre, difficulty=difficulty))
        return plugin

    async def generate_quest(self, genre, difficulty):
